...
ser = serial.Serial('COM5', 115200, xonxoff=False, rtscts=False, dsrdtr=False) #Tried with and without the last 3 parameters, and also at 1Mbps, same happens.
...
def check():
    ...
    if ser.isOpen() and ser.in_waiting:
        ser.flushInput()
        ser.flushOutput()
        data_raw = ser.readline().decode('ascii').rstrip('\r\n')
        print(data_raw)
        if(data_raw == b'\x001\r\n'):
            #Pausa
            Pause()
        else if (data_raw == b'\x002\r\n'):
            #Play
            Play()
        else if (data_raw == b'\x004\r\n'):
            #Next
            Next()
        else if (data_raw == b'\x008\r\n'):
            #Prev
            Prev()
